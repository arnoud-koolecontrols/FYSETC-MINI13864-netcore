...
namespace Iot.Device.Pn5180V2
{
    ...
    /// </summary>
    public class Pn5180 : CardTransceiver, IDisposable, ITranceiver
    {
        ...
        public int TransmitData(byte targetNumber, ReadOnlySpan<byte> dataToSend, int timeOutInMilliSeconds)
        {
            if (targetNumber == 0)
            {
        ...
        public int ReceiveData(byte targetNumber, out Span<byte> dataToReceive, int timeOutInMilliSeconds)
        {
            ...
            if (targetNumber == 0)
            {
                ...
                if (numBytes > 0)
                {
                    ReadDataFromCard(dataToReceive, dataToReceive.Length);
                }
        ...
        public int Tranceive(byte targetNumber, ReadOnlySpan<byte> dataToSend, out Span<byte> dataFromCard, int timeOutInMilliSeconds)
        {
            int result = -1;
            ...
            if (targetNumber == 0)
            {
                ...
                {
                    // Clears all interrupt
                    TransmitData(targetNumber, dataToSend, 2);
                    ReceiveData(targetNumber, out dataFromCard, 10);
                }
            }
            else
            {
